...
class ImdbFetcher:
  ...
  def __init__(self, tv_show):
    ...
    for i in range(1, int(no_seasons) + 1):
      ...
      if season_ratings != []:
        self.all_seasons.append(season_ratings)
        logging.info(f'Ratings Season {i}: {season_ratings}')
  
    self.max_episodes_per_season = max(list(map(lambda x: len(x), self.all_seasons)))
  ...
  def save_json(self):
    ...
    with open(f'output/{self.filename}.json', 'w') as json_file:
  ...
  def save_csv(self):
    with open(f'output/{self.filename}.csv', mode='w') as csv_file:
      ...
      w.writerow(['Season'] + [f'E{i:02}' for i in range(1, self.max_episodes_per_season + 1)])
      for s, season_ratings in enumerate(self.all_seasons):
