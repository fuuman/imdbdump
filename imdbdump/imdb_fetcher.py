...
parser.add_argument('-j', '--json', dest='json', action='store_true', default=False,
                    help='Dump fetched data to JSON file.')
...
class ImdbFetcher:
    ...
    def __init__(self, tv_show):
        ...
        self.title = r.html.find('#main > div.article.listo.list > div.subpage_title_block > div > h3 > a',
                                 first=True).text
        ...
        for i in range(1, int(no_seasons) + 1):
            ...
            r = session.get(url)
            season_ratings = [rating.find('span.ipl-rating-star__rating', first=True).text for rating in
                              r.html.find('div.ipl-rating-star')][0::23]
            if season_ratings != []:
